...
#path_y="C:\\Users\\Fabia\\Downloads\\y1.csv"   paths where your data lie
...
def sim_mu_fig1(d, n, sd_temp: float = 1.2, rng: np.random.Generator | None = None):
    ...
    Y = rng.normal(loc=0.0, scale=sd_temp, size=(n, d))  #just a (n,d) matrix of normal distributed values (with mean 0, std=sd_temp)
...
def sigma_spec_a(d, n, X, Y, bw, mmd_hat):  #should only be used inside of BFM-test
    ...
    s_a_squared = 4.0 * np.var(h1, ddof=1)  #ddof=1 means we use the sample variance, not the population variance (sample variance has a multiplier of 1/(n-1) and not 1/n  )
...
def BFM_test(d, n, epsilon, X, Y, Y2=None, bw=1.0, model_selection=False, level=0.05):
    ...
    if not model_selection:
        ...
        spec_reject_1 = (np.abs(spec_test_stat_1) > norm.ppf(1 - level/2))  #reject if the abs(spec_test_stat) > quantile_alpha of standard normal distribution

        spec_test_stat_q_1 = np.sqrt(n) * MMD_q / s_q
        spec_reject_q_1 = (np.abs(spec_test_stat_q_1) > norm.ppf(1 - level/2))  #reject if the abs(spec_test_stat_q_1) > quantile_alpha of standard normal distribution
        ...
        out = {
            ...
            "spec.reject.1": spec_reject_1, #for MMD_eps 1000 times is a vector and first column is for epsilon 1 and then count how many are 1 and divide by 1000 to get the graph (figure 1,2)
            "MMD.eps.1": MMD_eps,
            "MMD.hat.1": MMD_hat,
            "MMD.q.1": MMD_q,
...
#to test the BFM_test from the given data
n,d,theta1 = 1000, 16, 16
bw = np.sqrt(theta1 / 2)  #bw= sqrt( theta1 / 2)

#section to read in a given csv file
...
#data_y2=pd.read_csv(path_y2,index_col=0).to_numpy()
