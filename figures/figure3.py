...
def run_degenerate_case_simulation():
    ...
    sigmas = np.arange(1.0, 1.5, 0.1)   #we vary the std now
    d_values = [2, 16]
    nsim = 100
    level = 0.05
        ...
        for n in sample_sizes:
            ...
            for sigma in sigmas:
                ...
                for _ in tqdm(range(nsim), desc=f"d={d}, n={n}, sigma={sigma:.1f}"):
                    #in section 5.2 on page 20 for the degenerate U-statistic
                    data = sim_mu_2sampl(d, n, SD_theor1=(1.0, sigma), SD_theor2=(1.0, 1.0), rng=rng)   #for Y_1 we have on the second half of the sample a std of sigma with varying sigma
                    X = data['X']
                    Y1 = data['hat_Y1']
                    Y2 = data['hat_Y2']

                    result = BFM_test(d, n, epsilon, X, Y1, Y2, bw=bw, model_selection=True, level=level)   #now model_selection=True since we want to compare Y1 and Y2 to X
