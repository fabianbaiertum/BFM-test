...
for idx, d in enumerate([2, 16]):   #for d=2 and d=16 we simulate, can change it to any 2 numbers can extend it to any number, but have to adjust how many columns of plots we have
    ...
    for sd in sd_values:
        ...
        for _ in tqdm(range(n_simulations), desc=f"Simulations for d={d}, sd={sd}"):
            ...
            bfm_out = BFM_test(d, n, epsilon_list, X, Y, bw=bw, model_selection=False, level=level) #for each simulation we have to calculate the BFM_test and save the rejection values

            reject_q.append(bfm_out['spec.reject.q.1'])
            for i in range(3):
                reject_eps[i].append(bfm_out['spec.reject.1'][i])
