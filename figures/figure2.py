...
#to test the MMD from the given data
n,d,theta1 = 500, 16, 16
bw = np.sqrt(theta1 / 2)  #bw= sqrt( theta1 / 2)
...
def sim_sig_fig2(d, n, MU_temp=0, rng=None):
    """
    Python version of the R function SIM.sig.fig.2.
    to generate the data used in figure 2
    ...
    n : int
        Number of samples.
    MU_temp : float, optional
        Mean shift (default is 0).
    rng : np.random.Generator, optional
        Random number generator for reproducibility.
    ...
    -------
    dict
        A dictionary with keys 'X', 'Y', 'hat.Y', 'hat.sd.X'.
    """
    if rng is None:
        rng = np.random.default_rng()
    ...
    Y = rng.normal(0, 1, size=(n, d))

    # Estimate standard deviations for each column of X
    hat_sd_X = X.std(axis=0, ddof=1)  # sample standard deviation (ddof=1)

    # Create a matrix where each row is hat_sd_X
    matr_hat_sd = np.tile(hat_sd_X, (n, 1))

    # Compute hat.Y
    hat_Y = MU_temp + Y * matr_hat_sd  # broadcasting

    return {"X": X, "Y": Y, "hat.Y": hat_Y, "hat.sd.X": hat_sd_X}

sample_sizes = [100, 250, 500, 1000]
mean_shifts = np.arange(0.0, 0.7, 0.1)  #now we vary the mean
d_values = [2, 16]
...
for idx, d in enumerate(d_values):
    ...
    for n in sample_sizes:
        ...
        for mu_temp in mean_shifts:
            reject_q = []
            reject_eps = []

            for _ in tqdm(range(num_simulations), desc=f"Simulations for d={d}, n={n}, mean={mu_temp}"):
                data = sim_sig_fig2(d, n, MU_temp=mu_temp, rng=rng) #to generate the data sets
                X = data['X']
                Y = data['hat.Y']

                bfm_out = BFM_test(d, n, epsilon, X, Y, bw=bw, model_selection=False, level=level)  #for each simulation we calculate the BFM_test

                reject_q.append(bfm_out['spec.reject.q.1'])
                reject_eps.append(bfm_out['spec.reject.1'])

            results_mmdq.append(np.mean(reject_q))
            results_mmd_eps.append(np.mean(reject_eps))

        # Plot each n separately
        ax.plot(mean_shifts, results_mmdq, marker='o', label=f"MMD_q n={n}", linestyle='-')
        ax.plot(mean_shifts, results_mmd_eps, marker='o', label=f"eps n={n}", linestyle='--')
